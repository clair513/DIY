...
# Feed image blob into model for face detection:
...
for i in range(0, face.shape[2]):
    ci= face[0,0,i,2]
    if ci>args['conf']:
        bbox = face[0,0,i,3:7] * np.array([w,h,w,h])
        (tl,tr,bl,br) = bbox.astype('int')
        ...
        cv2.putText(img, text, (tl,y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,0,0), 2)
